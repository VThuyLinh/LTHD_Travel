...
const TourDetail = ({ navigation,route }) => {
    ...
    return (
           ...
           <ScrollView style={[MyStyle.container, MyStyle.margin]} >
            {tourdetail===null?<ActivityIndicator animating={true} color={'blue'} />:
                <Card style={MyStyle.bgrcolor}>
                    <Card.Content>
                        <Text variant="titleLarge" style={MyStyle.text3}>{tourdetail.Tour_Name}</Text>
                        <Text style={MyStyle.text4}>{tourdetail.Days} ngày {tourdetail.Nights==0 ?'':`${tourdetail.Nights} đêm`} </Text>
                    ...
                    <View>
                        ...
                        <Card >
                            <Card.Content>
                            {field.map(f => <TextInput onChangeText={t=> updateState(f.name,t) } key={f.name} style={MyStyle.margin} label={f.label} right={<TextInput.Icon icon={f.icon} />} />)}
                            </Card.Content>
